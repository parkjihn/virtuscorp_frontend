...
def get_gitignore_patterns(directory):
    ...
    ignore_patterns.update(
        ["package-lock.json", "README.md", "node_modules", "*.js", "*.map"]
    )
...
def combine_files_into_text(directory, output_file=None):
    ...
    ignore_patterns.add("node_modules")  # Ensure node_modules is always ignored
    ...
    for root, _, files in os.walk(directory):
        if "node_modules" in root.split(os.sep):  # Skip node_modules explicitly
            continue
